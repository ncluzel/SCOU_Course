...
def estimate_decorrelation_lag(series, confidence=0.95, max_lag=None):
    """
    """
    n = len(series)
    if max_lag is None:
    ...
    acf_vals, confint = acf(series, nlags=max_lag, alpha=1 - confidence, fft=True)
    noise_threshold = confint[1, 1] - acf_vals[1]

    # C1 — premier passage par "zéro"
    c1_thresh = noise_threshold#1.96 / np.sqrt(n)
    #c1_thresh = 0.4
    tau = next((k for k in range(1, max_lag + 1) if acf_vals[k] <= c1_thresh), max_lag)

    return tau

def draw_spaced_indices(n, tau, rng):
    """
    """
    origin = rng.integers(0, tau)
    return np.arange(origin, n, tau)
